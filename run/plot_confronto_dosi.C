void plot_confronto_dosi() {
    TCanvas *c_dosi = new TCanvas("c_dosi", "Dose: Riemann vs Gauss-Kronrod", 1000, 800);
    c_dosi->SetGrid();

    // Da profilo_dose.dat (Riemann) -> z (1), Dose (3)
    TGraph *g_dose_riemann = new TGraph("profilo_dose.dat", "%lg %*lg %lg");
    // Da profilo_dose_gk.dat -> z (1), TERMA (2), Dose_K_Coll (5), Kerma_Tot (6)
    TGraph *g_terma        = new TGraph("profilo_dose_gk.dat", "%lg %lg %*lg %*lg %*lg %*lg %*lg");
    TGraph *g_kerma        = new TGraph("profilo_dose_gk.dat", "%lg %*lg %lg %*lg %*lg %*lg %*lg");
    TGraph *g_kerma_tot    = new TGraph("profilo_dose_gk.dat", "%lg %*lg %*lg %*lg %*lg %lg %*lg");
    TGraph *g_dose_k_coll  = new TGraph("profilo_dose_gk.dat", "%lg %*lg %*lg %*lg %lg %*lg %*lg");
    TGraph *g_dose_k_tot   = new TGraph("profilo_dose_gk.dat", "%lg %*lg %*lg %*lg %*lg %*lg %lg");
    TGraph *g_dose_t_tot   = new TGraph("profilo_dose_gk.dat", "%lg %*lg %*lg %*lg %*lg %*lg %*lg %lg");
    TGraph *g_dose_t       = new TGraph("profilo_dose_gk.dat", "%lg %*lg %*lg %lg %*lg %*lg %*lg");
    TGraph *g_dose_k_tot_e = new TGraph("profilo_dose_gk.dat", "%lg %*lg %*lg %*lg %*lg %*lg %*lg %*lg %lg");
