void plot_kermas_smallStep() {
    TCanvas *c_kerma = new TCanvas("c_kerma_small", "Analisi KERMA e ODE", 800, 600);
    c_kerma->SetGrid();

    // 1 ODE (Colonna 5)
    TGraph *g_kerma_1ode = new TGraph("fluenza_terma_z_small.dat", "%lg %*lg %*lg %*lg %lg");
    // 2 ODE: p (2), s (3), tot (4)
    TGraph *g_kerma_p   = new TGraph("fluenza_prim_scat_small.dat", "%lg %lg %*lg %*lg %*lg %*lg");
    TGraph *g_kerma_s   = new TGraph("fluenza_prim_scat_small.dat", "%lg %*lg %lg %*lg %*lg %*lg");
    TGraph *g_kerma_tot = new TGraph("fluenza_prim_scat_small.dat", "%lg %*lg %*lg %lg %*lg %*lg");
